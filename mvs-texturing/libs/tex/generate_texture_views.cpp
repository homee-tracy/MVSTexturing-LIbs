...
void from_images_and_camera_files(std::string const& path,
  ...
  for (std::size_t i = 0; i < dir.size(); ++i) {
    ...
    std::string cam_file_ext =
        util::string::uppercase(util::string::right(cam_file.name, 4));
    if (cam_file_ext != ".CAM")
      continue;

    std::string prefix =
        util::string::left(cam_file.name, cam_file.name.size() - 4);
    if (prefix.empty())
      continue;
    ...
    for (std::size_t j = i + 1;; j += step) {
      ...
      if (j >= dir.size() ||
          util::string::left(dir[j].name, prefix.size()) != prefix) {
        if (step == 1) {
          j = i;
          step = -1;
          continue;
      ...
      std::string img_file_ext =
          util::string::uppercase(util::string::right(img_file.name, 4));
...
#pragma omp parallel for
  for (std::size_t i = 0; i < files.size(); i += 2) {
    ...
    if (!infile.good()) {
      throw util::FileException(util::fs::basename(cam_file),
                                std::strerror(errno));
    }
