...
template <typename C, typename R, typename T>
std::size_t SparseTable<C, R, T>::get_nnz(void) const {
  return nnz;
}
...
template <typename C, typename R, typename T>
typename SparseTable<C, R, T>::Column const& SparseTable<C, R, T>::col(
    C id) const {
  return column_wise_data[id];
}

template <typename C, typename R, typename T>
typename SparseTable<C, R, T>::Row const& SparseTable<C, R, T>::row(
    R id) const {
...
void SparseTable<C, R, T>::save_to_file(SparseTable const& sparse_table,
  ...
  out << TEX_SPARSE_TABLE_HEADER << " " << TEX_SPARSE_TABLE_VERSION << " "
      << cols << " " << rows << " " << nnz << std::endl;
