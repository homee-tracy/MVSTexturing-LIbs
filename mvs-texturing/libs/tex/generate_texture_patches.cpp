 ...
 */
TexturePatchCandidate generate_candidate(int label,
...
bool fill_hole(
    std::vector<std::size_t> const& hole, UniGraph const& graph,
...
#pragma omp critical(vpis)
  ...
  mve::FloatImage::Ptr image =
      mve::FloatImage::create(image_size, image_size, 3);
...
#pragma omp parallel for schedule(dynamic)
  for (std::size_t i = 0; i < texture_views->size(); ++i) {
    ...
    for (std::size_t j = 0; j < subgraphs.size(); ++j) {
      candidates.push_back(generate_candidate(label, *texture_view,
