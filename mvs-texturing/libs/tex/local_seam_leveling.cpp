...
math::Vec3f mean_color_of_edge_point(
  ...
  for (EdgeProjectionInfo const& edge_projection_info : edge_projection_infos) {
    ...
    if (texture_patch->get_label() == 0)
      continue;
    math::Vec2f pixel =
        edge_projection_info.p1 * t + (1.0f - t) * edge_projection_info.p2;
...
void draw_line(math::Vec2f p1, math::Vec2f p2,
  ...
  while (true) {
    ...
    float t =
        (length != 0.0f) ? std::sqrt(tdx * tdx + tdy * tdy) / length : 0.5f;
...
void local_seam_leveling(UniGraph const& graph,
  ...
  for (std::size_t i = 0; i < edge_projection_infos.size(); ++i) {
    ...
    for (std::size_t j = 0; j < edge_color.size(); ++j) {
      ...
      edge_color[j] = mean_color_of_edge_point(edge_projection_infos[i],
                                               *texture_patches, t);
    }
  ...
  for (std::size_t i = 0; i < vertex_colors.size(); ++i) {
    std::vector<VertexProjectionInfo> const& projection_infos =
        vertex_projection_infos[i];
    if (projection_infos.size() <= 1)
      continue;

    math::Accum<math::Vec3f> color_accum(math::Vec3f(0.0f));
    for (VertexProjectionInfo const& projection_info : projection_infos) {
      TexturePatch::Ptr texture_patch =
          texture_patches->at(projection_info.texture_patch_id);
      if (texture_patch->get_label() == 0)
        continue;
      math::Vec3f color =
          texture_patch->get_pixel_value(projection_info.projection);
      color_accum.add(color, 1.0f);
    }
    if (color_accum.w == 0.0f)
      continue;
    ...
    for (VertexProjectionInfo const& projection_info : projection_infos) {
      Pixel pixel;
      pixel.pos =
          math::Vec2i(projection_info.projection + math::Vec2f(0.5f, 0.5f));
      pixel.color = &vertex_colors[i];
      pixels[projection_info.texture_patch_id].push_back(pixel);
    }
  }

  ProgressCounter texture_patch_counter("\tBlending texture patches",
