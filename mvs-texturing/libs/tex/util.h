 ...
 */
template <typename T, int N>
T const multi_gauss_unnormalized(Eigen::Matrix<T, 1, N> const& X,
                                 Eigen::Matrix<T, 1, N> const& mu,
                                 Eigen::Matrix<T, N, N> const& covariance_inv) {

  Eigen::Matrix<T, 1, N> mean_removed = X - mu;
  return std::exp(T(-0.5) * mean_removed * covariance_inv *
                  mean_removed.adjoint());
}

/** Return the number suffix for n, e.g. 3 -> "rd". */
inline std::string number_suffix(int n) {
...
/**
  ...
  */
template <typename T>
void write_vector_to_csv(std::string const& filename,
...
void vector_to_file(std::string const& filename, std::vector<T> const& vector) {
  ...
  out.write(reinterpret_cast<const char*>(&vector[0]),
...
/**
  ...
  * @throws util::FileException
  */
inline void write_string_to_file(std::string const& filename,
