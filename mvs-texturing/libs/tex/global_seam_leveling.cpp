...
math::Vec3f sample_edge(TexturePatch::ConstPtr texture_patch, math::Vec2f p1,
                        math::Vec2f p2) {
...
void find_seam_edges_for_vertex_label_combination(
    UniGraph const& graph, mve::TriangleMesh::ConstPtr& mesh,
    mve::MeshInfo const& mesh_info, std::size_t vertex, std::size_t label1,
    std::size_t label2, std::vector<MeshEdge>* seam_edges) {
    ...
    for (std::size_t j = 0; j < edge_faces.size(); ++j) {
      for (std::size_t k = j + 1; k < edge_faces.size(); ++k) {
        ...
        if (!(face_label1 < face_label2))
          std::swap(face_label1, face_label2);

        if (face_label1 != label1 || face_label2 != label2)
          continue;
...
math::Vec3f calculate_difference(
  ...
  for (MeshEdge const& seam_edge : seam_edges) {
    ...
    for (EdgeProjectionInfo const& edge_projection_info :
         edge_projection_infos) {
      TexturePatch::Ptr texture_patch =
          texture_patches[edge_projection_info.texture_patch_id];
      ...
      if (texture_patch_label == label1 || texture_patch_label == label2) {
        if (texture_patch_label == label1)
          color1_accum.add(sample_edge(texture_patch, edge_projection_info.p1,
                                       edge_projection_info.p2),
                           length);

        if (texture_patch_label == label2)
          color2_accum.add(sample_edge(texture_patch, edge_projection_info.p1,
                                       edge_projection_info.p2),
                           length);
...
void global_seam_leveling(UniGraph const& graph,
                          mve::TriangleMesh::ConstPtr mesh,
      ...
      for (std::size_t k = 0; k < adj_verts.size(); ++k) {
        ...
        for (std::size_t l = 0; l < labels[adj_vertex].size(); ++l) {
          ...
          if (i < adj_vertex && label == adj_vertex_label) {
            Eigen::Triplet<float, int> t1(Gamma_row, vertlabel2row[i][label],
                                          lambda);
            Eigen::Triplet<float, int> t2(
                Gamma_row, vertlabel2row[adj_vertex][adj_vertex_label],
                -lambda);
    ...
    for (std::size_t j = 0; j < labels[i].size(); ++j) {
      for (std::size_t k = 0; k < labels[i].size(); ++k) {
        ...
        if (label1 < label2) {
          ...
          find_seam_edges_for_vertex_label_combination(
              graph, mesh, mesh_info, i, label1, label2, &seam_edges);
          ...
          coefficients_b.push_back(calculate_difference(
              vertex_projection_infos, mesh, *texture_patches, seam_edges,
              label1, label2));
  ...
  std::cout << "\tLhs dimensionality: " << Lhs.rows() << " x " << Lhs.cols()
            << std::endl;
...
#pragma omp critical
  ...
  std::cout << "\t\tTook " << timer.get_elapsed_sec() << " seconds"
            << std::endl;

  mve::TriangleMesh::FaceList const& mesh_faces = mesh->get_faces();

  ProgressCounter texture_patch_counter("\tAdjusting texture patches",
  ...
  for (std::size_t i = 0; i < texture_patches->size(); ++i) {
    ...
    for (std::size_t j = 0; j < faces.size(); ++j) {
      for (std::size_t k = 0; k < 3; ++k) {
        ...
        patch_adjust_values[j * 3 + k] =
            adjust_values[vertex].find(label)->second;
      }
    }
