...
Arguments parse_args(int argc, char** argv) {
  ...
  args.set_usage(
      ...
      " [options] IN_SCENE IN_MESH OUT_PREFIX"
      ...
      "\nFirst line: Extrinsics - translation vector and rotation matrix (the "
      "transform from world to camera)"
      "\nSecond line: Intrinsics - focal length, distortion coefficients, "
      "pixel aspect ratio and principal point"
      "\nThe focal length is the distance between camera center and image "
      "plane normalized by dividing with the larger image dimension."
      "\nFor non-zero distortion coefficients the image will be undistorted "
      "prior to the texturing process."
      " If only d0 is non-zero then the radial distortion model of VisualSFM "
      "is assumed, otherwise the Bundler distortion model is assumed."
      "\nThe pixel aspect ratio is usually 1 or close to 1. If your SfM system "
      "doesn't output it, but outputs a different focal length in x and y "
      "direction, you have to encode this here."
      "\nThe principal point has to be given in unit dimensions (e.g. 0.5 0.5)."
      "\n\nBUNDLE_FILE:"
      "\nCurrently only NVM bundle files (from VisualSFM, "
      "http://ccwu.me/vsfm/) are supported."
      "\nSince the bundle file contains relative paths to the images please "
      "make sure you did not move them (relative to the bundle) or rename them "
      "after the bundling process."
      "\n\nMVE_SCENE::EMBEDDING:"
      "\nThis is the scene representation we use in our research group: "
      "http://www.gris.tu-darmstadt.de/projects/multiview-environment/."
      "\n\nIN_MESH:"
      "\nThe mesh that you want to texture and which needs to be in the same "
      "coordinate frame as the camera parameters. You can reconstruct one, "
      "e.g. with CMVS: http://www.di.ens.fr/cmvs/"
      "\n\nOUT_PREFIX:"
      "\nA path and name for the output files, e.g. "
      "<path>/<to>/my_textured_mesh"
      "\nDon't append an obj extension. The application does that itself "
      "because it outputs multiple files (mesh, material file, texture files)."
      "\n");
  args.add_option('D', "data_cost_file", true,
                  "Skip calculation of data costs and use the ones provided in "
                  "the given file");
  args.add_option(
      'L', "labeling_file", true,
      "Skip view selection and use the labeling provided in the given file");
  args.add_option('d', "data_term", true,
                  "Data term: {" + choices<tex::DataTerm>() + "} [" +
                      choice_string<tex::DataTerm>(tex::DATA_TERM_GMI) + "]");
  args.add_option(
      's', "smoothness_term", true,
      "Smoothness term: {" + choices<tex::SmoothnessTerm>() + "} [" +
          choice_string<tex::SmoothnessTerm>(tex::SMOOTHNESS_TERM_POTTS) + "]");
  args.add_option(
      'o', "outlier_removal", true,
  ...
  args.add_option(
      't', "tone_mapping", true,
      "Tone mapping method: {" + choices<tex::ToneMapping>() + "} [" +
  ...
  args.add_option(
      '\0', SKIP_GEOMETRIC_VISIBILITY_TEST, false,
  ...
  for (util::ArgResult const* i = args.next_option(); i != 0;
       i = args.next_option()) {
    switch (i->opt->sopt) {
      case 'v':
      ...
      case 'd':
        ...
        break;
      case 's':
        conf.settings.smoothness_term =
            parse_choice<tex::SmoothnessTerm>(i->arg);
        break;
      case 'o':
        conf.settings.outlier_removal =
            parse_choice<tex::OutlierRemoval>(i->arg);
        break;
      case 't':
...
std::string Arguments::to_string() {
  ...
  out << "Input scene: \t" << in_scene << std::endl
      ...
      << "Data term: \t" << choice_string<tex::DataTerm>(settings.data_term)
      << std::endl
      << "Smoothness term: \t"
      << choice_string<tex::SmoothnessTerm>(settings.smoothness_term)
      << std::endl
      << "Outlier removal method: \t"
      << choice_string<tex::OutlierRemoval>(settings.outlier_removal)
      << std::endl
      << "Tone mapping: \t"
      << choice_string<tex::ToneMapping>(settings.tone_mapping) << std::endl
      << "Apply global seam leveling: \t"
      << bool_to_string(settings.global_seam_leveling) << std::endl
      << "Apply local seam leveling: \t"
      << bool_to_string(settings.local_seam_leveling) << std::endl;
