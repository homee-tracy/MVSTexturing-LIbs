...
VolumeMTAccessor::VolumeMTAccessor(void) : iter(-1) {}

/* ---------------------------------------------------------------- */

bool VolumeMTAccessor::next(void) {
...
void VolumeMTAccessor::load_new_cube(void) {
  ...
  math::Vec3f basepos(base_x * spacing - 0.5f, base_y * spacing - 0.5f,
                      base_z * spacing - 0.5f);
...
VolumeMCAccessor::VolumeMCAccessor(void) : iter(-1) {}

bool VolumeMCAccessor::next(void) {
  std::size_t const width = static_cast<std::size_t>(this->vol->width());
  std::size_t const height = static_cast<std::size_t>(this->vol->height());
  std::size_t const depth = static_cast<std::size_t>(this->vol->depth());
...
bool VolumeMCAccessor::has_colors(void) const {
  return false;
}
