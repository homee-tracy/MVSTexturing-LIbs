 ...
 */
template <>
unsigned char ply_read_value<unsigned char>(std::istream& input,
                                            PLYFormat format) {
  switch (format) {
    case PLY_ASCII: {
      int temp;
...
template unsigned int ply_read_value<unsigned int>(std::istream& input,
                                                   PLYFormat format);
...
TriangleMesh::Ptr load_ply_mesh(std::string const& filename) {
    ...
    if (header[0] == "format") {
      ...
      if (reading_verts) {
        ...
        /* List of accepted and handled attributes. */
          ...
          std::cout << "PLY Loader: Unrecognized vertex property \"" << buffer
                    << "\"" << std::endl;
          critical = true;
          continue;
        }
        ...
        if (header[1] == "list")
          ...
          std::cout << "PLY Loader: Unrecognized face property \"" << buffer
                    << "\"" << std::endl;
          critical = true;
        }
      } else if (reading_grid) {
        if (header[1] != "list") {
          std::cout << "PLY Loader: Unrecognized grid property \"" << buffer
                    << "\"" << std::endl;
          critical = true;
        }
        ...
        std::cout << "PLY Loader: Skipping property \"" << header[1]
  ...
  if (skip_bytes > 0) {
    std::cout << "PLY Loader: Skipping " << skip_bytes << " bytes."
              << std::endl;
        ...
        case PLY_F_VERTEX_INDICES: {
          ...
          if (n_verts == 3) {
            ...
            std::cout << "PLY Loader: Ignoring face with "
                      << static_cast<int>(n_verts) << " vertices!" << std::endl;
...
void save_ply_mesh(TriangleMesh::ConstPtr mesh, std::string const& filename,
  ...
  std::string format_str =
      (options.format_binary ? "binary_little_endian" : "ascii");
  ...
  std::cout << "Writing PLY file (" << verts.size() << " verts"
            ...
            << (write_vvalues ? ", with values" : "") << ", " << face_amount
            << " faces" << (write_fcolors ? ", with colors" : "")
...
void save_ply_view(std::string const& filename, CameraInfo const& camera,
                   FloatImage::ConstPtr depth_map,
                   FloatImage::ConstPtr confidence_map,
  ...
  if (confidence_map != nullptr &&
      (confidence_map->height() != height || confidence_map->width() != width))
    throw std::invalid_argument("Confidence map dimension does not match");

  if (color_image != nullptr &&
      (color_image->width() != width || color_image->height() != height))
  ...
  std::cout << "Writing PLY file for image size " << width << "x" << height
            << "..." << std::endl;
