...
void TriangleMesh::recalc_normals(bool face, bool vertex) {
  if (!face && !vertex)
    return;
  ...
  if (zlfn || zlvn)
    std::cout << "Warning: Zero-length normals detected: " << zlfn
              << " face normals, " << zlvn << " vertex normals" << std::endl;
}

/* ---------------------------------------------------------------- */

void TriangleMesh::ensure_normals(bool face, bool vertex) {
  bool need_face_recalc =
      (face && this->face_normals.size() != this->faces.size() / 3);
  bool need_vertex_recalc =
      (vertex && this->vertex_normals.size() != this->vertices.size());
  this->recalc_normals(need_face_recalc, need_vertex_recalc);
}

/* ---------------------------------------------------------------- */

void TriangleMesh::delete_vertices(DeleteList const& delete_list) {
...
void TriangleMesh::delete_vertices_fix_faces(DeleteList const& dlist) {
...
namespace {
bool is_valid_triangle(TriangleMesh::VertexID const* ids) {
  return ids[0] != ids[1] || ids[0] != ids[2];
}
}  // namespace

void TriangleMesh::delete_invalid_faces(void) {
...
std::size_t TriangleMesh::get_byte_size(void) const {
