...
inline ObjVertex::ObjVertex(void)
    : vertex_id(0), texcoord_id(0), normal_id(0) {}

inline bool ObjVertex::operator<(ObjVertex const& other) const {
...
void load_mtl_file(std::string const& filename,
  ...
  while (input.good()) {
    ...
    if (line[0] == "newmtl") {
      ...
      std::string path =
          util::fs::join_path(util::fs::dirname(filename), line[1]);
...
void load_obj_mesh(std::string const& filename,
                   std::vector<ObjModelPart>* obj_model_parts) {
  /* Precondition checks. */
  if (filename.empty())
    throw std::invalid_argument("No filename given");
  ...
  while (input.good()) {
    ...
    if (line[0] == "v") {
      ...
      std::cout << "OBJ Loader: Skipping unsupported element: " << line[0]
                << std::endl;
    }
  }
