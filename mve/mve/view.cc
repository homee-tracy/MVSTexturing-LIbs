...
void View::load_view(std::string const& user_path) {
...
void View::load_view_from_mve_file(std::string const& filename) {
  /* Open file. */
  ...
  while (true) {
    ...
    if (tokens[0] == "image" && tokens.size() == 6) {
      ...
      proxy.image = mve::image::create_for_type(proxy.type, proxy.width,
                                                proxy.height, proxy.channels);
      this->images.push_back(proxy);
      embedding_buffers.push_back(ReadBuffer(proxy.image->get_byte_size(),
      ...
      embedding_buffers.push_back(ReadBuffer(proxy.blob->get_byte_size(),
    ...
    } else if (tokens[0] == "camera-int" && tokens.size() >= 2 &&
               tokens.size() <= 7) {
...
void View::reload_view(void) {
  if (this->path.empty())
    throw std::runtime_error("View not initialized");
  this->load_view(this->path);
}

void View::save_view_as(std::string const& user_path) {
  std::string safe_path = util::fs::sanitize_path(user_path);
  safe_path = util::fs::abspath(safe_path);
...
std::size_t View::get_byte_size(void) const {
  std::size_t ret = 0;
  for (std::size_t i = 0; i < this->images.size(); ++i)
...
std::string View::get_value(std::string const& key) const {
  if (key.empty())
...
void View::set_value(std::string const& key, std::string const& value) {
  if (key.empty())
    throw std::invalid_argument("Empty key");
  if (key.find_first_of('.') == std::string::npos)
    throw std::invalid_argument("Missing section identifier");
...
void View::delete_value(std::string const& key) {
  this->meta_data.data.erase(key);
}

void View::set_camera(CameraInfo const& camera) {
...
ImageBase::Ptr View::get_image(std::string const& name, ImageType type) {
...
View::ImageProxy const* View::get_image_proxy(std::string const& name,
                                              ImageType type) {
  View::ImageProxy* proxy = this->find_image_intern(name);
  if (proxy != nullptr) {
    this->initialize_image(proxy, false);
...
bool View::has_image(std::string const& name, ImageType type) {
  View::ImageProxy* proxy = this->find_image_intern(name);
  if (proxy == nullptr)
    return false;
  if (type == IMAGE_TYPE_UNKNOWN)
...
void View::set_image(ImageBase::Ptr image, std::string const& name) {
  if (image == nullptr)
...
void View::set_image_ref(std::string const& filename, std::string name) {
...
bool View::remove_image(std::string const& name) {
...
ByteImage::Ptr View::get_blob(std::string const& name) {
...
View::BlobProxy const* View::get_blob_proxy(std::string const& name) {
  BlobProxy* proxy = this->find_blob_intern(name);
  if (proxy != nullptr)
...
bool View::has_blob(std::string const& name) {
  return this->find_blob_intern(name) != nullptr;
}

void View::set_blob(ByteImage::Ptr blob, std::string const& name) {
  if (blob == nullptr)
...
bool View::remove_blob(std::string const& name) {
...
void View::deprecated_format_check(std::string const& path) {
...
void View::load_meta_data(std::string const& path) {
...
void View::save_meta_data(std::string const& path) {
...
void View::populate_images_and_blobs(std::string const& path) {
  ...
  for (std::size_t i = 0; i < dir.size(); ++i) {
    ...
    if (name.empty()) {
      std::cerr << "View: Invalid file name " << file.name << ", skipping."
                << std::endl;
      continue;
    }
    ...
    if (ext4 == ".png" || ext4 == ".jpg" || ext5 == ".jpeg" ||
      ...
      std::cerr << "View: Unrecognized extension " << file.name << ", skipping."
                << std::endl;
    }
  }
}

void View::replace_file(std::string const& old_fn, std::string const& new_fn) {
  /* Delete old file. */
...
View::ImageProxy* View::find_image_intern(std::string const& name) {
  for (std::size_t i = 0; i < this->images.size(); ++i)
    if (this->images[i].name == name)
...
void View::initialize_image(ImageProxy* proxy, bool update) {
  if (proxy->is_initialized && !update)
    return;
  this->load_image_intern(proxy, true);
}

ImageBase::Ptr View::load_image(ImageProxy* proxy, bool update) {
...
void View::load_image_intern(ImageProxy* proxy, bool init_only) {
...
void View::save_image_intern(ImageProxy* proxy) {
  if (this->path.empty())
    throw std::runtime_error("View not initialized");
  if (proxy == nullptr)
  ...
  if (use_png_format)
    image::save_png_file(std::dynamic_pointer_cast<ByteImage>(proxy->image),
                         fname_new);
  else
...
View::BlobProxy* View::find_blob_intern(std::string const& name) {
  for (std::size_t i = 0; i < this->blobs.size(); ++i)
    if (this->blobs[i].name == name)
      return &this->blobs[i];
  return nullptr;
...
void View::initialize_blob(BlobProxy* proxy, bool update) {
  if (proxy->is_initialized && !update)
    return;
  this->load_blob_intern(proxy, true);
}

ByteImage::Ptr View::load_blob(BlobProxy* proxy, bool update) {
...
void View::load_blob_intern(BlobProxy* proxy, bool init_only) {
  if (this->path.empty())
    throw std::runtime_error("View not initialized");
  if (proxy->name.empty())
    return;
...
void View::save_blob_intern(BlobProxy* proxy) {
  if (this->path.empty())
    throw std::runtime_error("View not initialized");
...
void View::debug_print(void) {
  ...
  for (std::size_t i = 0; i < this->images.size(); ++i) {
    ...
    std::cout << "  " << proxy.name << " (" << proxy.filename << ")"
              << ", size " << proxy.width << "x" << proxy.height << "x"
              << proxy.channels << ", type " << proxy.type
