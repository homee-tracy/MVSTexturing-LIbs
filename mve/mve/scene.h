...
inline Scene::Scene(void) : bundle_dirty(false) {}

inline Scene::Ptr Scene::create(std::string const& path) {
  Scene::Ptr scene(new Scene);
  scene->load_scene(path);
  return scene;
}

inline Scene::ViewList const& Scene::get_views(void) const {
  return this->views;
}

inline Scene::ViewList& Scene::get_views(void) {
  return this->views;
}
...
inline std::string const& Scene::get_path(void) const {
  return this->basedir;
}

inline void Scene::set_bundle(Bundle::Ptr bundle) {
  this->bundle_dirty = true;
  this->bundle = bundle;
}

inline void Scene::reset_bundle(void) {
  this->bundle.reset();
