...
ByteImage::Ptr float_to_byte_image(FloatImage::ConstPtr image, float vmin,
                                   float vmax) {
  if (image == nullptr)
    throw std::invalid_argument("Null image given");
...
ByteImage::Ptr double_to_byte_image(DoubleImage::ConstPtr image, double vmin,
                                    double vmax) {
  if (image == nullptr)
    throw std::invalid_argument("Null image given");
...
ByteImage::Ptr raw_to_byte_image(RawImage::ConstPtr image, uint16_t vmin,
                                 uint16_t vmax) {
  if (image == nullptr)
    throw std::invalid_argument("Null image given");
...
void gamma_correct(ByteImage::Ptr image, float power) {
  ...
  for (int i = 0; i < 256; ++i)
    lookup[i] =
        static_cast<uint8_t>(std::pow(i / 255.0f, power) * 255.0f + 0.5f);
  for (int i = 0; i < image->get_value_amount(); ++i)
