...
inline View::View(void) {}

inline View::View(std::string const& path) {
  this->load_view(path);
}

inline View::Ptr View::create(void) {
  return Ptr(new View());
}

inline View::Ptr View::create(std::string const& path) {
  return Ptr(new View(path));
}

inline std::string const& View::get_directory(void) const {
  return this->path;
}

inline View::MetaData const& View::get_meta_data(void) const {
  return this->meta_data;
}

inline View::ImageProxies const& View::get_images(void) const {
  return this->images;
}

inline View::BlobProxies const& View::get_blobs(void) const {
  return this->blobs;
}

inline void View::set_id(int view_id) {
...
inline int View::get_id(void) const {
  std::string value = this->get_value("view.id");
  return value.empty() ? -1 : util::string::convert<int>(value, true);
}

inline std::string View::get_name(void) const {
  return this->get_value("view.name");
}

inline CameraInfo const& View::get_camera(void) const {
  return this->meta_data.camera;
}

inline bool View::is_camera_valid(void) const {
  return this->meta_data.camera.flen > 0.0f;
}

inline ByteImage::Ptr View::get_byte_image(std::string const& name) {
  return std::dynamic_pointer_cast<ByteImage>(
      this->get_image(name, IMAGE_TYPE_UINT8));
}

inline FloatImage::Ptr View::get_float_image(std::string const& name) {
