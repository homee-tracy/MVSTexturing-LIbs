...
void Scene::load_scene(std::string const& base_path) {
  if (base_path.empty())
...
bool Scene::is_dirty(void) const {
...
void Scene::cache_cleanup(void) {
  ...
  std::cout << "Cleanup: Released " << released << " embeddings in "
            << affected_views << " of " << total_views << " views."
            << std::endl;
}

/* ---------------------------------------------------------------- */

std::size_t Scene::get_total_mem_usage(void) {
  std::size_t ret = 0;
...
std::size_t Scene::get_view_mem_usage(void) {
  std::size_t ret = 0;
  for (std::size_t i = 0; i < this->views.size(); ++i)
    if (this->views[i] != nullptr)
...
void Scene::init_views(void) {
  ...
  /* Give some feedback... */
  std::cout << "Initializing scene with " << views_dir.size() << " views..."
            << std::endl;
  ...
  std::cout << "Initialized " << temp_list.size() << " views (max ID is "
            << max_id << "), took " << timer.get_elapsed() << "ms."
            << std::endl;
}

/* ---------------------------------------------------------------- */

Bundle::ConstPtr Scene::get_bundle(void) {
  if (this->bundle == nullptr) {
    std::string filename =
        util::fs::join_path(this->basedir, MVE_SCENE_BUNDLE_FILE);
