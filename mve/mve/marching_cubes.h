...
TriangleMesh::Ptr marching_cubes(T& accessor) {
  ...
  while (accessor.next()) {
    ...
    for (int i = 0; i < 12; ++i) {
      ...
      math::Vec3f x = math::interpolate(accessor.pos[ev[0]],
                                        accessor.pos[ev[1]], w[0], w[1]);

      if (accessor.has_colors()) {
        math::Vec3f col = math::interpolate(accessor.color[ev[0]],
                                            accessor.color[ev[1]], w[0], w[1]);
