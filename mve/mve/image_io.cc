...
void save_file(FloatImage::ConstPtr image, std::string const& filename) {
...
void save_file(FloatImage::Ptr image, std::string const& filename) {
...
namespace {
void load_png_headers_intern(FILE* fp, ImageHeaders* headers, png_structp* png,
                             png_infop* png_info) {
...
void save_png_file(ByteImage::ConstPtr image, std::string const& filename,
  ...
  for (int i = 0; i < image->height(); ++i)
    row_pointers[i] =
        const_cast<png_bytep>(&data[i * image->width() * image->channels()]);
...
void jpg_error_handler(j_common_ptr /*cinfo*/) {
  throw util::Exception("JPEG format not recognized");
}

void jpg_message_handler(j_common_ptr /*cinfo*/, int msg_level) {
  if (msg_level < 0)
...
void save_jpg_file(ByteImage::ConstPtr image, std::string const& filename,
                   int quality) {
  if (image == nullptr)
    throw std::invalid_argument("Null image given");
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    JSAMPROW row_pointer =
        const_cast<JSAMPROW>(&data[cinfo.next_scanline * row_stride]);
...
void tiff_error_handler(char const* /*module*/, char const* fmt, va_list ap) {
  char msg[2048];
  ::vsprintf(msg, fmt, ap);
...
void save_tiff_file(ByteImage::ConstPtr image, std::string const& filename) {
  ...
  int64_t ret =
      TIFFWriteEncodedStrip(tif, 0, buffer, image->get_value_amount());
...
void save_tiff_16_file(RawImage::ConstPtr image, std::string const& filename) {
  ...
  int64_t ret = TIFFWriteEncodedStrip(
      tif, 0, buffer, image->get_value_amount() * sizeof(uint16_t));
...
void save_ppm_file_intern(ImageBase::ConstPtr image,
...
namespace {
void load_mvei_headers_intern(std::istream& in, ImageHeaders* headers) {
...
ImageBase::Ptr load_mvei_file(std::string const& filename) {
  ...
  /* Load image data. */
  ImageBase::Ptr image = create_for_type(headers.type, headers.width,
                                         headers.height, headers.channels);
  in.read(image->get_byte_pointer(), image->get_byte_size());
  if (!in.good())
    throw util::FileException(filename, std::strerror(errno));
