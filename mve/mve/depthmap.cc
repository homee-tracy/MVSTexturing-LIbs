...
void depthmap_cleanup_grow(FloatImage::ConstPtr dm, FloatImage::Ptr ret,
                           std::vector<bool>& visited, std::size_t x,
                           std::size_t y, std::size_t thres) {
...
FloatImage::Ptr depthmap_cleanup(FloatImage::ConstPtr dm, std::size_t thres) {
...
float pixel_footprint(std::size_t x, std::size_t y, float depth,
...
math::Vec3f pixel_3dpos(std::size_t x, std::size_t y, float depth,
                        math::Matrix3f const& invproj) {
  math::Vec3f ray =
      invproj * math::Vec3f((float)x + 0.5f, (float)y + 0.5f, 1.0f);
  return ray.normalized() * depth;
}

/* ---------------------------------------------------------------- */

void dm_make_triangle(TriangleMesh* mesh, mve::Image<unsigned int>& vidx,
...
bool dm_is_depthdisc(float* widths, float* depths, float dd_factor, int i1,
                     int i2) {
...
TriangleMesh::Ptr depthmap_triangulate(FloatImage::ConstPtr dm,
                                       math::Matrix3f const& invproj,
                                       float dd_factor,
  ...
  for (int y = 0; y < height - 1; ++y, ++i) {
    for (int x = 0; x < width - 1; ++x, ++i) {
      ...
      float depths[4] = {dm->at(i, 0), dm->at(i + 1, 0), dm->at(i + width, 0),
                         dm->at(i + width + 1, 0)};
      ...
      int tris[4][3] = {{0, 2, 1}, {0, 3, 1}, {0, 2, 3}, {1, 2, 3}};
      ...
      if (dd_factor > 0.0f) {
        ...
        for (int j = 0; j < 4; ++j) {
          if (depths[j] == 0.0f)
            continue;
          widths[j] = pixel_footprint(x + (j % 2), y + (j / 2), depths[j],
                                      invproj);  // w, h, focal_len);
        }
...
#define DM_DD_ARGS widths, depths, dd_factor
          if (dm_is_depthdisc(DM_DD_ARGS, tv[0], tv[1]))
            tri[j] = 0;
          if (dm_is_depthdisc(DM_DD_ARGS, tv[1], tv[2]))
            tri[j] = 0;
          if (dm_is_depthdisc(DM_DD_ARGS, tv[2], tv[0]))
            tri[j] = 0;
        }
      }
...
void rangegrid_triangulate(Image<unsigned int> const& grid,
  ...
  for (int y = 0; y < h - 1; ++y)
    for (int x = 0; x < w - 1; ++x) {
      ...
      unsigned int vid[4] = {grid[i], grid[i + 1], grid[i + w],
                             grid[i + w + 1]};
...
#define ADDTRI(a, c, b)    \
  faces.push_back(vid[a]); \
  faces.push_back(vid[b]); \
  faces.push_back(vid[c])
          case 1:
            if (!DEPTHDISC(0, 2, 1)) {
              ADDTRI(0, 2, 1);
            }
