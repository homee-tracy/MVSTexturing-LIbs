...
math::Matrix3f get_rot_from_quaternion(double const* values) {
  math::Vec4f q(values);
  q.normalize();
...
Bundle::Ptr load_nvm_bundle(std::string const& filename,
...
Bundle::Ptr load_bundler_ps_intern(std::string const& filename,
                                   BundleFormat format) {
  std::ifstream in(filename.c_str());
  if (!in.good())
    throw util::FileException(filename, std::strerror(errno));
  ...
  if (num_views < 0 || num_views > 10000 || num_features < 0 ||
  ...
  std::cout << "Reading " << parser_string << " file (" << num_views
            << " cameras, " << num_features << " features)..." << std::endl;
  ...
  for (int i = 0; i < num_features; ++i) {
    ...
    if (in.eof()) {
      std::cerr << "Warning: Unexpected EOF (at feature " << i << ")"
                << std::endl;
      features.pop_back();
      break;
    }
...
void save_photosynther_bundle(Bundle::ConstPtr bundle,
  ...
  for (std::size_t i = 0; i < cameras.size(); ++i) {
    ...
    out << cam.trans[0] << " " << cam.trans[1] << " " << cam.trans[2] << "\n";
  }
