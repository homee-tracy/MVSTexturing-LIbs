 ...
 */
ByteImage::Ptr float_to_byte_image(FloatImage::ConstPtr image,
 ...
 * This is done by clamping values to [vmin, vmax] and scaling to [0, 255].
 */
ByteImage::Ptr double_to_byte_image(DoubleImage::ConstPtr image,
 ...
 * This is done by clamping values to [vmin, vmax] and scaling to [0, 255].
 */
ByteImage::Ptr raw_to_byte_image(RawImage::ConstPtr image, uint16_t vmin = 0,
                                 uint16_t vmax = 65535);
...
typename Image<DST>::Ptr type_to_type_image(
    typename Image<SRC>::ConstPtr image);

/**
 * Finds the smallest and largest value in the given image.
 */
template <typename T>
void find_min_max_value(typename mve::Image<T>::ConstPtr image, T* vmin,
                        T* vmax);
 ...
 */
template <typename T>
typename Image<T>::Ptr image_undistort_msps(typename Image<T>::ConstPtr img,
                                            double k0, double k1);
 ...
 */
template <typename T>
typename Image<T>::Ptr image_undistort_k2k4(typename Image<T>::ConstPtr img,
                                            double focal_length, double k2,
                                            double k4);
 ...
 */
template <typename T>
typename Image<T>::Ptr image_undistort_vsfm(typename Image<T>::ConstPtr img,
 ...
 */
template <typename T>
typename Image<T>::Ptr crop(typename Image<T>::ConstPtr image, int width,
                            int height, int left, int top, T const* fill_color);
 ...
 */
template <typename T>
typename Image<T>::Ptr rescale(typename Image<T>::ConstPtr image,
                               RescaleInterpolation interp, int width,
                               int height);
 ...
 */
template <typename T>
typename Image<T>::Ptr rescale_half_size(typename Image<T>::ConstPtr image);
 ...
 */
template <typename T>
typename Image<T>::Ptr rescale_half_size_gaussian(
    typename Image<T>::ConstPtr image, float sigma = 0.866025403784439f);
 ...
 */
template <typename T>
typename Image<T>::Ptr rescale_half_size_subsample(
    typename Image<T>::ConstPtr image);
 ...
 */
template <typename T>
typename Image<T>::Ptr rescale_double_size(typename Image<T>::ConstPtr img);
 ...
 */
template <typename T>
typename Image<T>::Ptr rescale_double_size_supersample(
    typename Image<T>::ConstPtr img);
 ...
 */
template <typename T>
void rescale_nearest(typename Image<T>::ConstPtr in,
 ...
 */
template <typename T>
void rescale_gaussian(typename Image<T>::ConstPtr in,
 ...
 */
template <typename T>
typename Image<T>::Ptr blur_gaussian(typename Image<T>::ConstPtr in,
                                     float sigma);
 ...
 */
template <typename T>
typename Image<T>::Ptr blur_boxfilter(typename Image<T>::ConstPtr in, int ks);
 ...
 */
template <typename T>
typename Image<T>::Ptr rotate(typename Image<T>::ConstPtr image,
                              RotateType type);
 ...
 */
template <typename T>
typename Image<T>::Ptr rotate(typename Image<T>::ConstPtr image, float angle,
                              T const* fill_color);
 ...
 */
template <typename T>
typename Image<T>::Ptr desaturate(typename Image<T>::ConstPtr image,
                                  DesaturateType type);
 ...
 */
template <typename T>
typename Image<T>::Ptr expand_grayscale(typename Image<T>::ConstPtr image);

/** Reduce alpha: Reduces RGBA or GA images to RGB or G images. */
template <typename T>
void reduce_alpha(typename mve::Image<T>::Ptr img);  // TODO Blend color?
 ...
 */
template <typename T>
typename mve::Image<T>::Ptr sobel_edge(typename mve::Image<T>::ConstPtr img);
 ...
 */
template <typename T>
typename Image<T>::Ptr subtract(typename Image<T>::ConstPtr i1,
 ...
 */
template <typename T>
typename Image<T>::Ptr difference(typename Image<T>::ConstPtr i1,
                                  typename Image<T>::ConstPtr i2);
...
typename Image<T_OUT>::Ptr integral_image(typename Image<T_IN>::ConstPtr image);
 ...
 */
template <typename T>
T integral_image_area(typename Image<T>::ConstPtr sat, int x1, int y1, int x2,
                      int y2, int cc = 0);
 ...
 */
template <typename T>
typename Image<T>::Ptr create_thumbnail(typename Image<T>::ConstPtr image,
...
void find_min_max_value(typename mve::Image<T>::ConstPtr image, T* vmin,
  ...
  *vmax = std::numeric_limits<T>::is_signed ? -std::numeric_limits<T>::max()
...
template <typename T>
typename Image<T>::Ptr rescale(typename Image<T>::ConstPtr img,
                               RescaleInterpolation interp, int width,
                               int height) {
  if (width == 0 && height == 0)
...
template <typename T>
typename Image<T>::Ptr rescale_half_size(typename Image<T>::ConstPtr img) {
    ...
    for (int x = 0; x < ow; ++x) {
      ...
      for (int c = 0; c < ic; ++c)
        out->at(outpos++) = math::interpolate<T>(
            ...
            img->at(ipix2 + c), img->at(ipix2 + ic * hasnext + c), 0.25f, 0.25f,
            0.25f, 0.25f);
    }
  }
...
template <typename T>
typename Image<T>::Ptr rescale_half_size_gaussian(
    typename Image<T>::ConstPtr img, float sigma) {
  int const iw = img->width();
  int const ih = img->height();
  int const ic = img->channels();
  int const ow = (iw + 1) >> 1;
  ...
  for (int y = 0; y < oh; ++y) {
    ...
    for (int x = 0; x < ow; ++x) {
      ...
      for (int c = 0; c < ic; ++c, ++i)
        out->at(i) = math::interpolate(px[0][c], px[1][c], px[2][c], px[3][c],
                                       w[0], w[1], w[2], w[3]);
    }
  }
...
typename Image<T>::Ptr rescale_double_size_supersample(
  ...
  for (int y = 0; y < oh; ++y) {
    ...
    for (int x = 0; x < ow; ++x) {
      ...
      T const* val[4] = {
          &img->at(yoff[0] + xoff[0], 0), &img->at(yoff[0] + xoff[1], 0),
          &img->at(yoff[1] + xoff[0], 0), &img->at(yoff[1] + xoff[1], 0)};

      for (int c = 0; c < ic; ++c, ++witer)
        out->at(witer) =
            math::interpolate(val[0][c], val[1][c], val[2][c], val[3][c], 0.25f,
                              0.25f, 0.25f, 0.25f);
    }
  }
...
template <typename T>
void rescale_nearest(typename Image<T>::ConstPtr img,
                     typename Image<T>::Ptr out) {
...
template <typename T>
void rescale_linear(typename Image<T>::ConstPtr img,
                    typename Image<T>::Ptr out) {
  if (img->channels() != out->channels())
    throw std::invalid_argument("Image channel mismatch");
...
template <typename T>
T gaussian_kernel(typename Image<T>::ConstPtr img, float x, float y, int c,
                  float sigma) {
...
template <typename T>
void rescale_gaussian(typename Image<T>::ConstPtr img,
...
typename Image<T>::Ptr rotate(typename Image<T>::ConstPtr image, float angle,
  ...
  for (int y = 0; y < h; ++y)
    for (int x = 0; x < w; ++x, ret_ptr += c) {
      ...
      if (sample_x < -0.5f || sample_x > w - 0.5f || sample_y < -0.5f ||
          sample_y > h - 0.5f)
...
template <typename T>
inline T desaturate_maximum(T const* v) {
  return *std::max_element(v, v + 3);
}

template <typename T>
inline T desaturate_lightness(T const* v) {
...
template <typename T>
inline T desaturate_luminosity(T const* v) {
  return math::interpolate(v[0], v[1], v[2], 0.21f, 0.72f, 0.07f);
}

template <typename T>
inline T desaturate_luminance(T const* v) {
...
template <typename T>
typename Image<T>::Ptr desaturate(typename Image<T>::ConstPtr img,
                                  DesaturateType type) {
  if (img == nullptr)
    throw std::invalid_argument("Null image given");
  ...
  switch (type) {
    ...
    case DESATURATE_LUMINOSITY:
      func = desaturate_luminosity<T>;
      break;
    case DESATURATE_LUMINANCE:
      func = desaturate_luminance<T>;
      break;
    case DESATURATE_AVERAGE:
      func = desaturate_average<T>;
      break;
    default:
      throw std::invalid_argument("Invalid desaturate type");
  }
...
typename mve::Image<T>::Ptr sobel_edge(typename mve::Image<T>::ConstPtr img) {
  ...
  typename mve::Image<T>::Ptr out = mve::Image<T>::create(width, height, chans);
  T* out_ptr = out->get_data_pointer();
...
template <typename T_IN, typename T_OUT>
typename Image<T_OUT>::Ptr integral_image(
    typename Image<T_IN>::ConstPtr image) {
  if (image == nullptr)
    throw std::invalid_argument("Null image given");
...
typename Image<T>::Ptr create_thumbnail(typename Image<T>::ConstPtr image,
  ...
  typename mve::Image<T>::Ptr thumb = mve::image::rescale<T>(
      image, mve::image::RESCALE_LINEAR, rescale_width, rescale_height);
  thumb = mve::image::crop<T>(thumb, thumb_width, thumb_height, crop_left,
                              crop_top, nullptr);
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x, out_ptr += chans) {
      ...
      if (k1 > 0.0) {
        ...
        std::complex<double> const t18 =
            (t17 + t14 / (t17 * 3.0)) *
            std::complex<double>(0.0, std::sqrt(3.0));
        std::complex<double> const t19 = -0.5 * (t17 + t18) + t14 / (t17 * 6.0);
        fx = t19.real() * fx / fy;
        fy = t19.real();
      }
