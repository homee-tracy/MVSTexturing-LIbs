...
void CameraInfo::fill_camera_translation(float* trans) const {
...
void CameraInfo::fill_world_to_cam_rot(float* mat) const {
  std::copy(this->rot, this->rot + 9, mat);
}

/* ---------------------------------------------------------------- */

void CameraInfo::fill_cam_to_world_rot(float* mat) const {
  mat[0] = rot[0];
  mat[1] = rot[3];
  mat[2] = rot[6];
  mat[3] = rot[1];
...
void CameraInfo::set_transformation(float const* mat) {
  rot[0] = mat[0];
  rot[1] = mat[1];
  rot[2] = mat[2];
  trans[0] = mat[3];
...
void CameraInfo::fill_calibration(float* mat, float width, float height) const {
  ...
  mat[0] = ax;
  mat[1] = 0.0f;
  mat[2] = width * this->ppoint[0];
  mat[3] = 0.0f;
  mat[4] = ay;
  mat[5] = height * this->ppoint[1];
  mat[6] = 0.0f;
  mat[7] = 0.0f;
  mat[8] = 1.0f;
}
...
void CameraInfo::fill_gl_projection(float* mat, float width, float height,
...
void CameraInfo::fill_inverse_calibration(float* mat, float width,
                                          float height) const {
  float dim_aspect = width / height;
  float image_aspect = dim_aspect * this->paspect;
  float ax, ay;
  ...
  mat[0] = 1.0f / ax;
  mat[1] = 0.0f;
  mat[2] = -width * this->ppoint[0] / ax;
  mat[3] = 0.0f;
  mat[4] = 1.0f / ay;
  mat[5] = -height * this->ppoint[1] / ay;
  mat[6] = 0.0f;
  mat[7] = 0.0f;
  mat[8] = 1.0f;
}
...
void CameraInfo::fill_reprojection(CameraInfo const& destination,
...
std::string CameraInfo::get_rotation_string(void) const {
  std::stringstream ss;
...
std::string CameraInfo::get_translation_string(void) const {
  std::stringstream ss;
  ss << std::setprecision(10);
  for (int i = 0; i < 3; ++i)
    ss << this->trans[i] << (i < 2 ? " " : "");
...
void CameraInfo::set_translation_from_string(std::string const& trans_string) {
  std::stringstream ss(trans_string);
  for (int i = 0; i < 3; ++i)
    ss >> this->trans[i];
}

void CameraInfo::set_rotation_from_string(std::string const& rot_string) {
...
void CameraInfo::debug_print(void) const {
