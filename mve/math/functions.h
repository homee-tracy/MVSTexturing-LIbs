 ...
 */
template <typename T>
inline T gaussian(T const& x, T const& sigma) {
 ...
 */
template <typename T>
inline T gaussian_xx(T const& xx, T const& sigma) {
 ...
 */
template <typename T>
inline T gaussian_2d(T const& x, T const& y, T const& sigma_x,
                     T const& sigma_y) {
...
template <typename T>
inline T interpolate(T const& v1, float w1) {
  return v1 * w1;
}

/** Specific interpolation (weighting) of a value for unsigned char. */
template <>
inline unsigned char interpolate(unsigned char const& v1, float w1) {
  return (unsigned char)((float)v1 * w1 + 0.5f);
}

/** Generic interpolation between two values. */
template <typename T>
inline T interpolate(T const& v1, T const& v2, float w1, float w2) {
  return v1 * w1 + v2 * w2;
}

/** Specific interpolation between two unsigned char values. */
template <>
inline unsigned char interpolate(unsigned char const& v1,
                                 unsigned char const& v2, float w1, float w2) {
  return (unsigned char)((float)v1 * w1 + (float)v2 * w2 + 0.5f);
}

/** Generic interpolation between three values. */
template <typename T>
inline T interpolate(T const& v1, T const& v2, T const& v3, float w1, float w2,
                     float w3) {
...
inline unsigned char interpolate(unsigned char const& v1,
                                 ...
                                 unsigned char const& v3, float w1, float w2,
                                 float w3) {
  return (unsigned char)((float)v1 * w1 + (float)v2 * w2 + (float)v3 * w3 +
                         0.5f);
}
...
inline unsigned char interpolate(unsigned char const& v1,
                                 ...
                                 unsigned char const& v4, float w1, float w2,
                                 float w3, float w4) {
  return (unsigned char)((float)v1 * w1 + (float)v2 * w2 + (float)v3 * w3 +
                         (float)v4 * w4 + 0.5f);
}
...
std::size_t inline popcount(std::uint32_t const x) {
#if !(__amd64__ || __x86_64__ || _WIN64 || _M_X64)
  __pragma(
      message("Warning: CPU support for _mm_popcnt_u32 is assumed, "
              "but could not be verified."))
#endif
      return static_cast<std::size_t>(_mm_popcnt_u32(x));
}

std::size_t inline popcount(std::uint64_t const x) {
#if __amd64__ || __x86_64__ || _WIN64 || _M_X64
...
inline int to_gray_code(int bin) {
  return bin ^ (bin >> 1);
}

inline int from_gray_code(int gc) {
  int b = 0, ret = 0;
  for (int i = 31; i >= 0; --i) {
    b = (b + ((gc & (1 << i)) >> i)) % 2;
    ret |= b << i;
