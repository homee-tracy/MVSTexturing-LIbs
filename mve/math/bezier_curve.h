...
template <class T>
inline void BezierCurve<T>::append_point(T const& p) {
  this->cp.push_back(p);
}

template <class T>
inline T BezierCurve<T>::evaluate(float t) const {
  if (this->cp.size() < 2)
...
template <class T>
inline void BezierCurve<T>::clear(void) {
  this->cp.clear();
}

template <class T>
inline std::size_t BezierCurve<T>::size(void) const {
  return this->cp.size();
}

template <class T>
inline T const& BezierCurve<T>::operator[](std::size_t index) const {
