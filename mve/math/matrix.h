...
template <typename T, int N, int M>
inline Matrix<T, N, M>::Matrix(void) {}

template <typename T, int N, int M>
inline Matrix<T, N, M>::Matrix(T const* values) {
  std::copy(values, values + N * M, m);
}

template <typename T, int N, int M>
inline Matrix<T, N, M>::Matrix(T const& value) {
  fill(value);
}

template <typename T, int N, int M>
inline Matrix<T, N, M>::Matrix(Matrix<T, N, M> const& other) {
  std::copy(*other, *other + M * N, m);
}

template <typename T, int N, int M>
template <typename O>
inline Matrix<T, N, M>::Matrix(Matrix<O, N, M> const& other) {
  std::copy(*other, *other + M * N, m);
}
...
template <typename T, int N, int M>
inline bool matrix_is_square(Matrix<T, N, M> const& /*m*/) {
  return false;
}

template <typename T, int N>
inline bool matrix_is_square(Matrix<T, N, N> const& /*m*/) {
  return true;
}

template <typename T, int N>
inline Matrix<T, N, N>& matrix_inplace_transpose(Matrix<T, N, N>& matrix) {
...
template <typename T, int N, int M>
inline Matrix<T, N, M>& Matrix<T, N, M>::fill(T const& value) {
  std::fill(m, m + N * M, value);
  return *this;
}

template <typename T, int N, int M>
inline bool Matrix<T, N, M>::is_square(void) const {
  return matrix_is_square(*this);
}

template <typename T, int N, int M>
inline T Matrix<T, N, M>::minimum(void) const {
  return *std::min_element(m, m + N * M);
}

template <typename T, int N, int M>
inline T Matrix<T, N, M>::maximum(void) const {
  return *std::max_element(m, m + N * M);
}

template <typename T, int N, int M>
inline Vector<T, M> Matrix<T, N, M>::row(int index) const {
  return Vector<T, M>(m + index * M);
}

template <typename T, int N, int M>
inline Vector<T, N> Matrix<T, N, M>::col(int index) const {
...
template <typename T, int N, int M>
template <int O>
inline Matrix<T, N, M + O> Matrix<T, N, M>::hstack(
    Matrix<T, N, O> const& other) const {
...
template <typename T, int N, int M>
template <int O>
inline Matrix<T, N + O, M> Matrix<T, N, M>::vstack(
    Matrix<T, O, M> const& other) const {
  Matrix<T, N + O, M> ret;
...
template <typename T, int N, int M>
inline Matrix<T, N, M + 1> Matrix<T, N, M>::hstack(
    Vector<T, N> const& other) const {
  Matrix<T, N, M + 1> ret;
  T const* in1 = m;
  T const* in2 = *other;
...
template <typename T, int N, int M>
inline Matrix<T, N + 1, M> Matrix<T, N, M>::vstack(
    Vector<T, M> const& other) const {
  Matrix<T, N + 1, M> ret;
  std::copy(m, m + M * N, *ret);
...
template <typename T, int N, int M>
inline Matrix<T, N - 1, M> Matrix<T, N, M>::delete_row(int index) const {
  Matrix<T, N - 1, M> ret;
...
template <typename T, int N, int M>
inline Matrix<T, N, M - 1> Matrix<T, N, M>::delete_col(int index) const {
  Matrix<T, N, M - 1> ret;
  T const* in_ptr = this->begin();
  T* out_ptr = ret.begin();
...
template <typename T, int N, int M>
inline Matrix<T, N, M>& Matrix<T, N, M>::negate(void) {
  std::for_each(m, m + N * M, &algo::foreach_negate_value<T>);
  return *this;
}

template <typename T, int N, int M>
inline Matrix<T, N, M> Matrix<T, N, M>::negated(void) const {
  return Matrix<T, N, M>(*this).negate();
}

template <typename T, int N, int M>
inline Matrix<T, M, N>& Matrix<T, N, M>::transpose(void) {
  return matrix_inplace_transpose(*this);
}

template <typename T, int N, int M>
inline Matrix<T, M, N> Matrix<T, N, M>::transposed(void) const {
  Matrix<T, M, N> ret;
  for (int i = 0; i < N; ++i)
    for (int j = 0; j < M; ++j)
      ret(j, i) = (*this)(i, j);
...
template <typename T, int N, int M>
template <int U>
inline Matrix<T, N, U> Matrix<T, N, M>::mult(Matrix<T, M, U> const& rhs) const {
  typedef algo::InterleavedIter<T, U> RowIter;
  Matrix<T, N, U> ret;
  for (int i = 0; i < ret.rows; ++i)
    for (int j = 0; j < ret.cols; ++j)
      ret(i, j) =
          std::inner_product(m + M * i, m + M * i + M, RowIter(*rhs + j), T(0));
  return ret;
}

template <typename T, int N, int M>
inline Vector<T, N> Matrix<T, N, M>::mult(Vector<T, M> const& rhs) const {
  Vector<T, N> ret;
  for (int i = 0; i < N; ++i)
...
template <typename T, int N, int M>
inline Vector<T, N - 1> Matrix<T, N, M>::mult(Vector<T, M - 1> const& rhs,
                                              T const& v) const {
  Vector<T, N - 1> ret;
  for (int i = 0; i < N - 1; ++i)
...
template <typename T, int N, int M>
inline bool Matrix<T, N, M>::is_similar(Matrix<T, N, M> const& other,
                                        T const& eps) const {
...
template <typename T, int N, int M>
inline T* Matrix<T, N, M>::begin(void) {
  return m;
}

template <typename T, int N, int M>
inline T const* Matrix<T, N, M>::begin(void) const {
  return m;
}

template <typename T, int N, int M>
inline T* Matrix<T, N, M>::end(void) {
  return m + N * M;
}

template <typename T, int N, int M>
inline T const* Matrix<T, N, M>::end(void) const {
  return m + N * M;
}
...
template <typename T, int N, int M>
inline T* Matrix<T, N, M>::operator*(void) {
  return m;
}

template <typename T, int N, int M>
inline T const* Matrix<T, N, M>::operator*(void) const {
  return m;
}

template <typename T, int N, int M>
inline T& Matrix<T, N, M>::operator()(int row, int col) {
  return m[row * M + col];
}

template <typename T, int N, int M>
inline T const& Matrix<T, N, M>::operator()(int row, int col) const {
  return m[row * M + col];
}
template <typename T, int N, int M>
inline T& Matrix<T, N, M>::operator[](unsigned int i) {
  return m[i];
}

template <typename T, int N, int M>
inline T const& Matrix<T, N, M>::operator[](unsigned int i) const {
  return m[i];
}

template <typename T, int N, int M>
inline bool Matrix<T, N, M>::operator==(Matrix<T, N, M> const& rhs) const {
...
template <typename T, int N, int M>
inline Matrix<T, N, M>& Matrix<T, N, M>::operator=(Matrix<T, N, M> const& rhs) {
...
template <typename T, int N, int M>
template <typename O>
inline Matrix<T, N, M>& Matrix<T, N, M>::operator=(Matrix<O, N, M> const& rhs) {
  std::copy(*rhs, *rhs + N * M, m);
  return *this;
}

template <typename T, int N, int M>
inline Matrix<T, N, M> Matrix<T, N, M>::operator-(void) const {
  return negated();
}

template <typename T, int N, int M>
inline Matrix<T, N, M>& Matrix<T, N, M>::operator-=(
...
template <typename T, int N, int M>
template <int U>
inline Matrix<T, N, U> Matrix<T, N, M>::operator*(
    Matrix<T, M, U> const& rhs) const {
  return mult(rhs);
}

template <typename T, int N, int M>
inline Vector<T, N> Matrix<T, N, M>::operator*(Vector<T, M> const& rhs) const {
  return mult(rhs);
}
...
template <typename T, int N, int M>
inline Matrix<T, N, M>& Matrix<T, N, M>::operator-=(T const& rhs) {
  std::for_each(m, m + N * M, algo::foreach_substraction_with_const<T>(rhs));
  return *this;
}

template <typename T, int N, int M>
inline Matrix<T, N, M> Matrix<T, N, M>::operator-(T const& rhs) const {
  return Matrix<T, N, M>(*this) -= rhs;
}
...
template <typename T, int N, int M>
inline std::ostream& operator<<(std::ostream& os, Matrix<T, N, M> const& m) {
