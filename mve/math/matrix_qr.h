 ...
 */
template <typename T>
void matrix_qr(T const* mat_a, int rows, int cols, T* mat_q, T* mat_r,
...
void matrix_qr(Matrix<T, M, N> const& mat_a, Matrix<T, M, M>* mat_q,
 ...
 */
template <typename T>
void matrix_givens_rotation(T const& alpha, T const& beta, T* givens_c,
                            T* givens_s, T const& epsilon) {
 ...
 */
template <typename T>
void matrix_apply_givens_column(T* mat, int rows, int cols, int givens_i,
                                int givens_k, T const& givens_c,
                                T const& givens_s) {
 ...
 */
template <typename T>
void matrix_apply_givens_row(T* mat, int /*rows*/, int cols, int givens_i,
                             int givens_k, T const& givens_c,
                             T const& givens_s) {
    ...
    for (int i = rows - 1; i > j; --i) {
      ...
      for (int k = 0; k < submatrix_width; ++k) {
        matrix_subset[0 * submatrix_width + k] =
            mat_r[(i - 1) * cols + (j + k)];
        matrix_subset[1 * submatrix_width + k] = mat_r[i * cols + (j + k)];
      }
