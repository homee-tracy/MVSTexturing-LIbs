...
Value const* binary_search(std::vector<std::pair<Key, Value>> const& vec,
                           Key const& key) {
  std::size_t range1 = 0;
...
struct IncrementGenerator {
  ...
  T operator()(void) { return this->state++; }
};
...
template <typename T>
inline T accum_squared_sum(T const& init, T const& next) {
  return init + next * next;
}

/** Absolute sum accumulator. */
template <typename T>
inline T accum_absolute_sum(T const& init, T const& next) {
 ...
 */
template <typename T>
void vector_clean(std::vector<bool> const& delete_list,
                  std::vector<T>* vector) {
 ...
 */
template <typename T>
inline void kernel_region(T const& cx, T const& cy, T const& ks, T const& width,
                          T const& height, T* x1, T* x2, T* y1, T* y2) {
  *x1 = (cx > ks ? cx - ks : T(0));
...
template <typename T>
inline void sort_values(T* a, T* b, T* c) {
  if (*b < *a)
    std::swap(*a, *b);
  if (*c < *b)
    std::swap(*b, *c);
