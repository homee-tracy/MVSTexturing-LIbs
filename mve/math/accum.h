...
template <typename T>
inline Accum<T>::Accum(void) : w(0.0f) {}

template <typename T>
inline Accum<T>::Accum(T const& init) : v(init), w(0.0f) {}

template <typename T>
inline void Accum<T>::add(T const& value, float weight) {
  this->v += value * weight;
  this->w += weight;
}

template <typename T>
inline void Accum<T>::sub(T const& value, float weight) {
  this->v -= value * weight;
  this->w -= weight;
}

template <typename T>
inline T Accum<T>::normalized(float weight) const {
  return this->v / weight;
}

template <typename T>
inline T Accum<T>::normalized(void) const {
...
inline Accum<unsigned char>::Accum(void) : w(0.0f) {}

inline Accum<unsigned char>::Accum(unsigned char const& init)
    : v(init), w(0.0f) {}

inline void Accum<unsigned char>::add(unsigned char const& value,
                                      float weight) {
  this->v += static_cast<float>(value) * weight;
  this->w += weight;
}

inline void Accum<unsigned char>::sub(unsigned char const& value,
                                      float weight) {
  this->v -= static_cast<float>(value) * weight;
  this->w -= weight;
}

inline unsigned char Accum<unsigned char>::normalized(float weight) const {
  return static_cast<unsigned char>(math::round(this->v / weight));
}

inline unsigned char Accum<unsigned char>::normalized(void) const {
