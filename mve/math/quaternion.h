...
template <typename T>
inline Quaternion<T>::Quaternion(T const& value) : Vector<T, 4>(value) {}

template <typename T>
inline Quaternion<T>::Quaternion(T const& v1, T const& v2, T const& v3,
                                 T const& v4) {
  this->v[0] = v1;
  this->v[1] = v2;
  this->v[2] = v3;
...
template <typename T>
inline Quaternion<T>::Quaternion(Vector<T, 3> const& axis, T const& angle) {
  this->set(axis, angle);
}

template <typename T>
inline void Quaternion<T>::set(Vector<T, 3> const& axis, T const& angle) {
...
template <typename T>
inline void Quaternion<T>::set_from_rotation_matrix(T const* rot) {
...
template <typename T>
void Quaternion<T>::get_axis_angle(T* axis, T& angle) {
  T len = this->norm();
  if (len == T(0)) {
    axis[0] = T(1);
    axis[1] = T(0);
...
template <typename T>
void Quaternion<T>::to_rotation_matrix(T* matrix) const {
...
template <typename T>
Vector<T, 3> Quaternion<T>::rotate(Vector<T, 3> const& vec) const {
  T rot[9];
...
template <typename T>
inline Quaternion<T> Quaternion<T>::operator*(Quaternion<T> const& rhs) const {
  return Quaternion<T>(this->v[0] * rhs.v[0] - this->v[1] * rhs.v[1] -
...
template <typename T>
inline Quaternion<T>& Quaternion<T>::operator*=(Quaternion<T> const& rhs) {
  *this = *this * rhs;
  return *this;
}
...
template <typename T>
inline Quaternion<T> Quaternion<T>::conjugated(void) const {
