 ...
 */
template <typename T>
void matrix_svd(T const* mat_a, int rows, int cols, T* mat_u, T* vec_s,
                T* mat_v,
...
void matrix_svd(Matrix<T, M, N> const& mat_a, Matrix<T, M, N>* mat_u,
 ...
 */
template <typename T, int M, int N>
void matrix_pseudo_inverse(
    Matrix<T, M, N> const& A, Matrix<T, N, M>* result,
    T const& epsilon = T(MATH_SVD_DEFAULT_ZERO_THRESHOLD));
 ...
 */
template <typename T>
bool matrix_is_submatrix_zero_enclosed(T const* mat, int m, int k,
                                       T const& epsilon) {
  int const j = m - k - 1;
  if (j < 0)
    return true;
 ...
 */
template <typename T>
bool matrix_is_superdiagonal_nonzero(T const* mat, int rows, int cols,
                                     T const& epsilon) {
 ...
 */
template <typename T>
void matrix_householder_vector(T const* input, int length, T* vector, T* beta,
                               T const& epsilon, T const& norm_factor) {
  T sigma(0);
 ...
 */
template <typename T>
void matrix_householder_matrix(T const* vector, int length, T const beta,
                               T* matrix) {
 ...
 */
template <typename T>
void matrix_apply_householder_matrix(T* mat_a, int rows, int cols,
                                     T const* house_mat, int house_length,
  ...
  for (int i = 0; i < house_length; ++i) {
    for (int j = 0; j < house_length_n; ++j) {
      rhs[i * house_length_n + j] =
          mat_a[(offset_rows + i) * cols + (offset_cols + j)];
    }
  }
  ...
  for (int i = 0; i < (rows - offset_rows); ++i) {
    for (int j = 0; j < (cols - offset_cols); ++j) {
      ...
      for (int k = 0; k < house_length; ++k) {
        current +=
            (house_mat[i * house_length + k] * rhs[k * house_length_n + j]);
      }
 ...
 */
template <typename T>
void matrix_bidiagonalize(T const* mat_a, int rows, int cols, T* mat_u,
  ...
  for (int k = 0; k < steps; ++k) {
    ...
    matrix_householder_vector(input_vec, sub_length, house_vec, &house_beta,
                              epsilon, T(1));
    matrix_householder_matrix(house_vec, sub_length, house_beta, house_mat);
    matrix_apply_householder_matrix(mat_b, rows, cols, house_mat, sub_length, k,
                                    k);
    ...
    if (k <= cols - 3) {
      ...
      std::vector<T> buffer2(inner_sub_length    // inner_input_vec
                             ...
                             + inner_sub_length *
                                   inner_sub_length     // inner_house_mat
      ...
      matrix_householder_vector(inner_input_vec, inner_sub_length,
                                inner_house_vec, &inner_house_beta, epsilon,
                                norm);
      ...
      matrix_multiply(mat_b_tmp, slice_rows, slice_cols, inner_house_mat,
                      inner_sub_length, mat_b_res);
 ...
 */
template <typename T>
void matrix_gk_svd_step(int rows, int cols, T* mat_b, T* mat_q, T* mat_p, int p,
                        int q, T const& epsilon) {
  ...
  matrix_multiply(mat_b22, slice_length, slice_length, mat_b22_t, slice_length,
                  mat_tmp);
  ...
  for (int k = p; k < cols - q - 1; ++k) {
    ...
    matrix_apply_givens_column(mat_b, cols, cols, k, k + 1, givens_c, givens_s);
    matrix_apply_givens_column(mat_p, cols, cols, k, k + 1, givens_c, givens_s);

    alpha = mat_b[k * cols + k];
    beta = mat_b[(k + 1) * cols + k];
    matrix_givens_rotation(alpha, beta, &givens_c, &givens_s, epsilon);
    internal::matrix_apply_givens_row(mat_b, cols, cols, k, k + 1, givens_c,
                                      givens_s);
    internal::matrix_apply_givens_column(mat_q, rows, cols, k, k + 1, givens_c,
                                         givens_s);
...
template <typename T>
void matrix_svd_clear_super_entry(int rows, int cols, T* mat_b, T* mat_q,
  ...
  for (int i = row_index + 1; i < cols; ++i) {
    ...
    T norm =
        MATH_POW2(mat_b[row_index * cols + i]) + MATH_POW2(mat_b[i * cols + i]);
    norm = std::sqrt(norm) * MATH_SIGN(mat_b[i * cols + i]);

    T givens_c = mat_b[i * cols + i] / norm;
    T givens_s = mat_b[row_index * cols + i] / norm;
    matrix_apply_givens_row(mat_b, cols, cols, row_index, i, givens_c,
                            givens_s);
    matrix_apply_givens_column(mat_q, rows, cols, row_index, i, givens_c,
                               givens_s);
  }
}
 ...
 */
template <typename T>
void matrix_gk_svd(T const* mat_a, int rows, int cols, T* mat_u, T* vec_s,
                   T* mat_v, T const& epsilon) {
  ...
  std::vector<T> buffer(mat_q_full_size + mat_b_full_size + mat_p_size +
                        mat_q_size + mat_b_size);
  ...
  matrix_bidiagonalize(mat_a, rows, cols, mat_q_full, mat_b_full, mat_p,
                       epsilon);
  ...
  while (iteration < max_iterations) {
    ...
    for (int k = 0; k < cols; ++k) {
      ...
      if (matrix_is_diagonal(&mat_b33[0], slice_len, slice_len, epsilon)) {
        if (k < cols - 1) {
          if (matrix_is_submatrix_zero_enclosed(mat_b, cols, k + 1, epsilon)) {
            q = k + 1;
          }
        } else {
          q = k + 1;
    ...
    for (int k = 0; k < (cols - q); ++k) {
      ...
      if (matrix_is_superdiagonal_nonzero(mat_b22_tmp, slice_len, slice_len,
                                          epsilon)) {
        z = k + 1;
      }
    }
 ...
 */
template <typename T>
void matrix_r_svd(T const* mat_a, int rows, int cols, T* mat_u, T* vec_s,
                  T* mat_v, T const& epsilon) {
  /* Allocate memory for temp matrices. */
...
void matrix_svd(T const* mat_a, int rows, int cols, T* mat_u, T* vec_s,
  ...
  if (rows >= cols) {
    ...
    if (rows >= 5 * cols / 3) {
      internal::matrix_r_svd(mat_a, rows, cols, mat_u, vec_s, mat_v, epsilon);
    } else {
      internal::matrix_gk_svd(mat_a, rows, cols, mat_u, vec_s, mat_v, epsilon);
    }
  } else {
    ...
    internal::matrix_gk_svd(&mat_a_tmp[0], cols, cols, &mat_u_tmp[0], vec_s,
                            mat_v, epsilon);
...
void matrix_svd(Matrix<T, M, N> const& mat_a, Matrix<T, M, N>* mat_u,
  ...
  matrix_svd<T>(mat_a.begin(), M, N, mat_u_ptr, mat_s_ptr, mat_v_ptr, epsilon);
...
template <typename T, int M, int N>
void matrix_pseudo_inverse(Matrix<T, M, N> const& A, Matrix<T, N, M>* result,
                           T const& epsilon) {
  Matrix<T, M, N> U;
  Matrix<T, N, N> S;
  Matrix<T, N, N> V;
