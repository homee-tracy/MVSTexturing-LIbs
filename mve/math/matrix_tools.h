 ...
 */
template <typename T>
Matrix<T, 4, 4> matrix_gl_projection(T const& znear, T const& zfar,
 ...
 */
template <typename T>
Matrix<T, 4, 4> matrix_inverse_gl_projection(T const& znear, T const& zfar,
                                             T const& top, T const& right);
 ...
 */
template <typename T>
Matrix<T, 4, 4> matrix_viewtrans(Vector<T, 3> const& campos,
 ...
 */
template <typename T>
Matrix<T, 4, 4> matrix_inverse_viewtrans(Vector<T, 3> const& campos,
                                         Vector<T, 3> const& viewdir,
                                         Vector<T, 3> const& upvec);
 ...
 */
template <typename T>
bool matrix_is_diagonal(T* const mat, int rows, int cols,
                        T const& epsilon = T(0));
 ...
 */
template <typename T>
Matrix<T, 3, 3> matrix_rotation_from_axis_angle(Vector<T, 3> const& axis,
                                                T const& angle);
 ...
 */
template <typename T>
void matrix_multiply(T const* mat_a, int rows_a, int cols_a, T const* mat_b,
...
template <typename T, int N>
bool matrix_is_identity(Matrix<T, N, N> const& mat, T const& epsilon) {
...
template <typename T, int N>
inline T matrix_trace(math::Matrix<T, N, N> const& mat) {
  T ret(0.0);
  for (int i = 0; i < N * N; i += N + 1)
    ret += mat[i];
  return ret;
...
template <typename T>
inline T matrix_determinant(Matrix<T, 1, 1> const& mat) {
  return mat[0];
}

template <typename T>
inline T matrix_determinant(Matrix<T, 2, 2> const& mat) {
  return mat[0] * mat[3] - mat[1] * mat[2];
}

template <typename T>
inline T matrix_determinant(Matrix<T, 3, 3> const& m) {
...
template <typename T>
inline Matrix<T, 1, 1> matrix_inverse(Matrix<T, 1, 1> const& mat) {
  return matrix_inverse(mat, matrix_determinant(mat));
}

template <typename T>
inline Matrix<T, 2, 2> matrix_inverse(Matrix<T, 2, 2> const& mat) {
  return matrix_inverse(mat, matrix_determinant(mat));
}

template <typename T>
inline Matrix<T, 3, 3> matrix_inverse(Matrix<T, 3, 3> const& m) {
  return matrix_inverse(m, matrix_determinant(m));
}

template <typename T>
Matrix<T, 1, 1> matrix_inverse(Matrix<T, 1, 1> const& /*mat*/, T const& det) {
...
template <typename T>
bool matrix_is_diagonal(T* const mat, int rows, int cols, T const& epsilon) {
...
template <typename T>
void matrix_swap_columns(T* mat, int rows, int cols, int c1, int c2) {
