...
inline BSpline<V, T>::BSpline(void) : n(3) {}

template <class V, class T>
bool BSpline<V, T>::empty(void) const {
  return this->points.empty();
}

template <class V, class T>
inline void BSpline<V, T>::set_degree(int degree) {
  this->n = degree;
}

template <class V, class T>
inline int BSpline<V, T>::get_degree(void) const {
  return this->n;
}

template <class V, class T>
inline void BSpline<V, T>::reserve(std::size_t n_points) {
  this->points.reserve(n_points);
  this->knots.reserve(n_points + n + 1);
}

template <class V, class T>
inline void BSpline<V, T>::add_point(V const& p) {
  this->points.push_back(p);
}

template <class V, class T>
inline void BSpline<V, T>::add_knot(T const& t) {
  this->knots.push_back(t);
}

template <class V, class T>
inline void BSpline<V, T>::uniform_knots(T const& min, T const& max) {
  T width = max - min;
...
template <class V, class T>
inline void BSpline<V, T>::scale_knots(T const& min, T const& max) {
...
template <class V, class T>
inline typename BSpline<V, T>::PointVector const& BSpline<V, T>::get_points(
    void) const {
  return this->points;
}

template <class V, class T>
inline typename BSpline<V, T>::KnotVector const& BSpline<V, T>::get_knots(
    void) const {
  return this->knots;
}

template <class V, class T>
inline V BSpline<V, T>::evaluate(T const& t) const {
  /* FIXME inefficient */
...
template <class V, class T>
inline T BSpline<V, T>::deboor(int i, int k, T const& x) const {
  if (k == 0) {
    //FIXME
...
template <class V, class T>
inline void BSpline<V, T>::transform(math::Matrix4f const& transf) {
