...
class Logging {
 public:
  enum LogLevel { LOG_ERROR, LOG_WARNING, LOG_INFO, LOG_VERBOSE, LOG_DEBUG };
...
template <typename T>
inline Logging::NullStream& Logging::NullStream::operator<<(T const& /*arg*/) {
  return *this;
}

inline Logging::Logging(void) : max_level(LOG_INFO) {}

inline Logging::Logging(LogLevel max_level) : max_level(max_level) {}

inline void Logging::set_max_level(LogLevel max_level) {
  this->max_level = max_level;
}

inline std::ostream& Logging::log(LogLevel log_level) const {
  if (log_level > this->max_level)
    return const_cast<NullStream&>(this->nullstream);
  return (log_level == LOG_ERROR) ? std::cerr : std::cout;
}

inline std::ostream& Logging::error(void) const {
  return this->log(LOG_ERROR);
}

inline std::ostream& Logging::warning(void) const {
  return this->log(LOG_WARNING);
}

inline std::ostream& Logging::info(void) const {
  return this->log(LOG_INFO);
}

inline std::ostream& Logging::verbose(void) const {
  return this->log(LOG_VERBOSE);
}

inline std::ostream& Logging::debug(void) const {
