...
template <typename T, size_t alignment>
inline AlignedAllocator<T, alignment>::AlignedAllocator(void) {}

template <typename T, size_t alignment>
template <class T_other, size_t alignment_other>
inline AlignedAllocator<T, alignment>::AlignedAllocator(
...
template <typename T, size_t alignment>
inline void AlignedAllocator<T, alignment>::deallocate(pointer p,
                                                       size_type /*n*/) {
#ifdef _WIN32
  ::_aligned_free(p);
#else
...
template <typename T, size_t alignment>
inline void AlignedAllocator<T, alignment>::construct(pointer p,
                                                      const_reference other) {
  ::new ((void*)p) T(other);
}

template <typename T, size_t alignment>
inline void AlignedAllocator<T, alignment>::destroy(pointer p) {
  p->~T();
}

template <typename T, size_t alignment>
template <class T_other, size_t alignment_other>
inline bool AlignedAllocator<T, alignment>::operator==(
