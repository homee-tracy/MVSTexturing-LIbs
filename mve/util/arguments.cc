...
void Arguments::set_usage(char const* argv0, std::string const& usage) {
  std::stringstream ss;
...
void Arguments::add_option(char shortname, std::string const& longname,
...
void Arguments::parse(int argc,
                      char const* const* argv) throw(util::Exception) {
...
void Arguments::parse(std::vector<std::string> const& args) throw(
    util::Exception) {
  try {
...
void Arguments::parse_long_opt(std::string const& tok) {
...
bool Arguments::parse_short_opt(std::string const& tok1,
                                std::string const& tok2) {
  if (tok1.size() < 2)
...
ArgOption const* Arguments::find_opt(char sopt) {
  for (std::size_t i = 0; i < this->options.size(); ++i)
...
ArgOption const* Arguments::find_opt(std::string const& lopt) {
  for (std::size_t i = 0; i < this->options.size(); ++i)
...
void Arguments::parse_intern(std::vector<std::string> const& args) {
  this->results.clear();
...
ArgResult const* Arguments::next_result(void) {
  this->cur_result += 1;
...
ArgResult const* Arguments::next_option(void) {
  while (true) {
    this->cur_result += 1;
    if (this->cur_result >= this->results.size())
      break;
...
std::string Arguments::get_nth_nonopt(std::size_t index) {
  std::size_t cnt = 0;
  for (std::size_t i = 0; i < this->results.size(); ++i) {
...
void Arguments::get_ids_from_string(std::string const& str,
                                    std::vector<int>* ids) {
  ids->clear();
...
void Arguments::generate_helptext(std::ostream& stream) const {
  std::string descr =
      util::string::wordwrap(this->descr_str.c_str(), this->descrtext_width);
  ...
  for (std::size_t i = 0; i < this->options.size(); ++i) {
    ...
    stream << "  " << std::setiosflags(std::ios::left)
           << std::setw(this->helptext_indent) << optstr.str() << opt.desc
           << std::endl;
  }
}
