...
class Arguments {
 public:
  ...
  void parse(std::vector<std::string> const& args) throw(util::Exception);
  /** Parses command line arguments. */
  void parse(int argc, char const* const* argv) throw(util::Exception);
...
template <typename T>
inline T ArgResult::get_arg(void) const {
  return string::convert<T>(this->arg);
}

template <>
inline std::string ArgResult::get_arg(void) const {
  return this->arg;
}

inline void Arguments::set_usage(std::string const& str) {
  this->usage_str = str;
}

inline void Arguments::set_description(std::string const& str) {
  this->descr_str = str;
}

inline void Arguments::set_description_word_wrap(int width) {
  this->descrtext_width = width;
}

inline void Arguments::set_helptext_indent(int indent) {
  this->helptext_indent = indent;
}

inline void Arguments::set_nonopt_maxnum(std::size_t limit) {
  this->nonopt_max = limit;
}

inline void Arguments::set_nonopt_minnum(std::size_t limit) {
  this->nonopt_min = limit;
}

inline void Arguments::set_exit_on_error(bool exit) {
  this->auto_exit = exit;
}

template <typename T>
T Arguments::get_nth_nonopt_as(std::size_t index) {
