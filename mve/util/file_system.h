...
class FileLock {
 ...
 public:
  ...
  /**
     ...
     */
  Status acquire_retry(std::string const& filename, int retries = 50,
                       int sleep = 100);
  ...
  /**
     ...
     */
  bool wait_lock(std::string const& filename, int retries = 50,
                 int sleep = 100);
...
inline File::File(void) : is_dir(false) {}

inline File::File(std::string const& path, std::string const& name, bool isdir)
    : path(path), name(name), is_dir(isdir) {}

inline Directory::Directory(void) {}

inline Directory::Directory(std::string const& path) {
  this->scan(path);
}

inline FileLock::FileLock(void) {}

inline FileLock::~FileLock(void) {
  this->release();
}

inline std::string const& FileLock::get_reason(void) const {
  return this->reason;
}
