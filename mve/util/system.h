...
// void print_build_timestamp(char const* application_name, char const* date,
//                            char const* time);
...
template <int N>
inline void byte_swap(char* data);

/** Little endian to host order conversion. */
template <typename T>
inline T letoh(T const& x) {
  return x;
}

/** Big endian to host order conversion. */
template <typename T>
inline T betoh(T const& x) {
  return x;
}

/* ---------------------------------------------------------------- */

inline void sleep(std::size_t msec) {
  std::this_thread::sleep_for(std::chrono::milliseconds(msec));
}

inline void sleep_sec(float secs) {
...
inline void rand_seed(int seed) {
  std::srand(seed);
}
...
// inline void print_build_timestamp(char const* application_name) {
...
// template <>
// inline void byte_swap<2>(char* data) {
//   std::swap(data[0], data[1]);
// }

// template <>
// inline void byte_swap<4>(char* data) {
//   std::swap(data[0], data[3]);
//   std::swap(data[1], data[2]);
// }

// template <>
// inline void byte_swap<8>(char* data) {
