...
inline WallTimer::WallTimer(void) {
  this->reset();
}

inline void WallTimer::reset(void) {
  this->start = TimerClock::now();
}

inline std::size_t WallTimer::get_elapsed(void) const {
  TimerDurationMs diff = TimerClock::now() - this->start;
  return diff.count();
}

inline float WallTimer::get_elapsed_sec(void) const {
  return static_cast<float>(this->get_elapsed()) / 1000.0f;
}

/* ---------------------------------------------------------------- */

inline ClockTimer::ClockTimer(void) {
  this->reset();
}

inline void ClockTimer::reset(void) {
  this->start = ClockTimer::now();
}

inline float ClockTimer::now_sec(void) {
  return (float)std::clock() / (float)CLOCKS_PER_SEC;
}

inline std::size_t ClockTimer::now(void) {
  return ((std::size_t)(std::clock()) * 1000) / (std::size_t)CLOCKS_PER_SEC;
}

inline float ClockTimer::get_elapsed_sec(void) const {
  return (1.0f / 1000.0f) * (float)this->get_elapsed();
}

inline std::size_t ClockTimer::get_elapsed(void) const {
