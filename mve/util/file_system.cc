...
void write_string_to_file(std::string const& data,
                          std::string const& filename) {
  write_string_to_file(&data[0], data.size(), filename);
}

void write_string_to_file(char const* data, std::size_t len,
                          std::string const& filename) {
...
std::string File::get_absolute_name(void) const {
#ifdef _WIN32
  // FIXME: Use '/' even on windows?
  return (!path.empty() && *path.rbegin() == '\\' ? path + name
                                                  : path + "\\" + name);
#else
  return (!path.empty() && *path.rbegin() == '/' ? path + name
                                                 : path + "/" + name);
...
void Directory::scan(std::string const& path) {
  this->clear();
...
FileLock::FileLock(std::string const& filename) {
  switch (this->acquire_retry(filename)) {
    case LOCK_CREATED:
      break;
    default:
      throw util::Exception(this->reason);
  }
}

FileLock::Status FileLock::acquire(std::string const& filename) {
...
FileLock::Status FileLock::acquire_retry(std::string const& filename,
                                         int retries, int sleep) {
...
bool FileLock::is_locked(std::string const& filename) {
  std::string lockfname = filename + ".lock";
  return fs::file_exists(lockfname.c_str());
}

bool FileLock::wait_lock(std::string const& filename, int retries, int sleep) {
