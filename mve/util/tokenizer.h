...
inline void Tokenizer::split(std::string const& str, char delim,
                             bool keep_empty) {
  this->clear();
...
inline void Tokenizer::parse_cmd(std::string const& str) {
  this->clear();
  bool in_quote = false;
  std::string token;
...
inline std::string Tokenizer::concat(std::size_t pos, std::size_t num) const {
  std::stringstream ss;
  std::size_t max =
      (num == 0 ? this->size() : std::min(pos + num, this->size()));
...
template <typename T>
inline T Tokenizer::get_as(std::size_t pos) const {
